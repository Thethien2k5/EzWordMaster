...
fun FlipCardScreen(
    ...
    Box(
        ...
        Column(
            ...
            // Chỉ báo tiến độ
            Text(
                text = "Đã ghép: ${UI_STATE.MATCHED_PAIRS}/${UI_STATE.CARDS.size / 2}",
                fontSize = 16.sp,
            ...
            LazyVerticalGrid(
                columns = GridCells.Fixed(
                    when {
                        UI_STATE.CARDS.size <= 4 -> 2
                        UI_STATE.CARDS.size <= 9 -> 3
                        UI_STATE.CARDS.size <= 16 -> 4
                        else -> 5
                    }
                ),
                ...
                items(UI_STATE.CARDS, key = { it.id }) { card ->
                    FlipCardItem(
                        card = card,
                        isFlipped = UI_STATE.FLIPPED_CARDS.any { it.id == card.id },
                        isMatched = card.isMatched,
                        // Sửa lại để lấy đúng trạng thái từ ViewModel
                        isWrong = UI_STATE.WRONG_CARD_IDS.contains(card.id),
                        isCorrect = UI_STATE.CORRECT_CARD_IDS.contains(card.id),
                        totalCards = UI_STATE.CARDS.size,
                        onClick = {
