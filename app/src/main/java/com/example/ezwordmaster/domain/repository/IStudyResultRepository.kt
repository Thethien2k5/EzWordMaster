...
interface IStudyResultRepository {
     ...
     */
    suspend fun createStudyResultsFileIfMissing()

    /**
     * Tải tất cả kết quả học tập từ nguồn dữ liệu.
     */
    suspend fun loadStudyResults(): StudyResultsList

    /**
     * Thêm một kết quả học tập mới vào nguồn dữ liệu.
     */
    suspend fun addStudyResult(newResult: StudyResult)

    /**
     * Lấy danh sách kết quả học tập theo một chế độ học cụ thể.
     */
    suspend fun getStudyResultsByMode(studyMode: String): List<StudyResult>

    /**
     * Lấy danh sách kết quả học tập theo một chủ đề cụ thể.
     */
    suspend fun getStudyResultsByTopic(topicId: String): List<StudyResult>

    /**
     * Lấy danh sách kết quả học tập được sắp xếp theo thời gian (mới nhất trước).
     */
    suspend fun getStudyResultsSortedByTime(): List<StudyResult>
     ...
     */
    suspend fun getTodayStudyProgress(topicId: String): TodayProgress

    /**
     * THÊM VÀO: Lấy tổng tiến trình học của TẤT CẢ chủ đề trong ngày
     */
    suspend fun getOverallTodayProgress(): TodayProgress
}